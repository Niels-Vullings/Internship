...
#Improved functions
The functions below are improved and 
...
#   
#   #Dyad combinations
#   stable00 <- which(jumpst1 == 0 & jumpst2 == 0, arr.ind = TRUE)
#   stable01 <- which(flips == 0 & stablet1 == 1 & stablet2 == 1, arr.ind = TRUE)
#   stable11 <- which(jumpst1 == 2 & jumpst2 == 2, arr.ind = TRUE)
#   flip <- which(flips == 2, arr.ind = TRUE) #flips, so 01 at T1 and 10 at T2
#   jump02 <- which(jumpst1 == 0 & jumpst2 == 2, arr.ind = TRUE) #Null jump, from Null to Mutual
#   jump20 <- which(jumpst1 == 2 & jumpst2 == 0, arr.ind = TRUE) #Mutual jump, from Mutual to Null
#   
#   
...
# dyad.census.type(s501,s502, save.dyads = T)
...
things to fix:
  - If a dyad type is not present, the code will crash because there is an empty dataframe
```{r}
ts_dyad.census <- function(t1,t2, sim_sample = c(1:1000), simN = 5, seed = 345657343, save.dyads = FALSE){
  ...
  dyad.census.type <- function(t1, t2, save.dyads = FALSE){
    ...
    if(save.dyads == TRUE){
      ...
      stable00$tie <- "Null > Null"
      stable01$tie <- "Assym > Assym"
      stable11$tie <- "Mutual > Mutual"
      Null_Assym$tie <- "Null > Assym"
      Assym_Null$tie <- "Assym > Null"
      Assym_Mut$tie <- "Assym > Mutual"
      Mut_Assym$tie <- "Mutual > Assym"
      flip$tie <- "Tie flip"
      jump02$tie <- "Null > Mutual"
      jump20$tie <- "Mutual > Null"
  ...
  if(is.list(t2)){
    ...
    df <- foreach::foreach(sim_sample, i=iterators::icount(), .combine="rbind") %dopar% {
      df <- as.data.frame(dyad.census.type(s501, t2= t2[[i]])) #loop the dyad.census.type function over all relevant simulations and save in df
    }
...
# ts_dyad.census(s501,ts_ansSp3$simnets, sim_sample = c(1:1000), simN = 1000)
...
# check <- reshape2::melt((s501))
# check$Var2 <- as.numeric(check$Var2)

# check <- check %>% select(Var1, Var2)
dyads_check <- dyads 

# check <- check %>% mutate(check1 = paste0(Var1, Var2))

dyads_check <- dyads_check %>% mutate(check1 = as.numeric(paste0(row, col)))
dyads_check <- dyads_check %>% mutate(check2 = as.numeric(paste0(col, row)))
checkje <- dyads_check %>% count(check1)
checkje2 <- dyads_check %>% count(check2)

check <- check %>% mutate(dif = if_else(check1 %in% dyads_check$check1, TRUE, FALSE))

check %>% count(dif)

dyads_check <- dyads_check %>% mutate(dif = if_else(check1 %in% check$check1, TRUE, FALSE))
